...
def get_upcoming_events():
    ...
    upcoming_events = [event for event in sorted_events if datetime.strptime(event['date'], '%Y-%m-%d').date() >= today]
...
def get_past_events():
    ...
    # Filter past events
    past_events = [event for event in list_events if datetime.strptime(event['date'], '%Y-%m-%d').date() < today]
...
def receive_login_data():
    ...
    user = next((user for user in login_data if user['username'] == username and user['password'] == password), None)
...
def func_upcoming_events():
  ...
  completion = client.chat.completions.create(
    model="gpt-3.5-turbo",
    messages=[
        {"role": "user", "content": "Here is a list of events you will have to analyze " + str(upcoming_events)},
        {"role": "user", "content": "Here is information about a person's interets. Look at ALL the intersts to see what type of things they like. " + str(user_interest)},
        {"role": "assistant", "content": "Just return a list of the events from the provided events (with name, date, and description in dictionary format) that this person would be interested in based on their interests and likes. Please make sure to look at all their interests. No other text."}
    ]
    )   
...
def func_past_events():
  ...
  completion = client.chat.completions.create(
    model="gpt-3.5-turbo",
    messages=[
        {"role": "user", "content": "Here is a list of events you will have to analyze " + str(past_events)},
        {"role": "user", "content": "Here is information about a person's interets. Look at ALL the intersts to see what type of things they like. " + str(user_interest)},
        {"role": "assistant", "content": "Just return a list of the events from the provided events (with name, date, and description in dictionary format) that this person would be interested in based on their interests and likes. Please make sure to look at all their interests. No other text."}
    ]
    )   
